...
Important Reminders:
- Look into restricting PGD to a **subset of layers** instead of every parameter. The motivation is memory: it is currently unclear whether 12B (and even 9B) recovery training fits on a single GPU, especially once `validate_sparsity=True` walks every masked param every step. A reasonable cutoff is "only PGD layers downstream of the deepest SAE we actually use on the `aruna` branch":
    - **Gemma-2 9B** (42 layers, indexed 0-41): PGD only layers **32-41** (i.e. *after* layer 31). Cutoff comes from `gemma-scope-9b-it-res-canonical` SAE `layer_31/width_16k/canonical`, which is the deepest 9B SAE used in `aruna`'s scoping pipelines.
    - **Gemma-3 12B** (48 layers, indexed 0-47): PGD only layers **42-47** (i.e. *after* layer 41). Cutoff comes from `gemma-scope-2-12b-it-res` SAE `layer_41_width_16k_l0_medium`, which is the deepest 12B SAE used in `aruna`'s scoping/eval/attack scripts. Alternatively, consider the more aggressive cutoff *after layer 31* — `gemma-scope-2-12b-it-res` also has `layer_31_width_16k_l0_medium` (and `layer_31_width_16k_l0_small`), so layers **32-47** would still be downstream of an available SAE and would free up a lot more memory.
    - Implementation hook: `build_pgd_masks_from_model` already takes a `param_names` allowlist — pass only the late-layer names. Add a validator that asserts no mask is installed for a parameter belonging to a layer index <= the cutoff.
