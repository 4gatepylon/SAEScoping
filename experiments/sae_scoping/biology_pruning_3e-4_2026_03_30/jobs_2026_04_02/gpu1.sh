#!/bin/bash
# CUDA device 3 (physical GPU 3): SAE physics, vanilla chemistry, SAE math
set -uo pipefail
cd "$(dirname "$0")/.."
...
python script_train_gemma9b_sae.py \
    ...
    || echo "[CUDA:3] FAILED: SAE physics"

echo "=== [CUDA:3] Vanilla chemistry ==="
python script_train_gemma9b_sae.py \
    ...
    || echo "[CUDA:3] FAILED: vanilla chemistry"

echo "=== [CUDA:3] SAE math ==="
python script_train_gemma9b_sae.py \
    --sae --subset math \
    -c "$CKPT" -p "$DIST" -h 0.0003 \
    -b 2 -a 8 -s 2000 \
