...
class _HfCheckpointCallback(TrainerCallback):
    """Captures the wandb run ID and uploads each checkpoint to HF."""
    def __init__(self):
...
ALL_DOMAINS = ["coding", "biology", "chemistry", "math", "cyber"]
...
def load_domain_train_eval(domain: str, tokenizer: PreTrainedTokenizerBase, eval_fraction=0.2, seed=42) -> tuple[Dataset, Dataset]:
    ...
    elif domain == "cyber": full = _load_wmdp_cyber_raw(1987, tokenizer)
