...
Stages:
  ...
  3. RECOVER:  In-domain SFT on camel-ai/biology + MegaScience biology
...
def load_stemqa_biology_for_ranking(
...
def load_biology_train_dataset(tokenizer: PreTrainedTokenizerBase) -> Dataset:
    """camel-ai/biology + MegaScience biology for in-domain recovery (streaming)."""
...
def stage_rank(
    ...
    """Stage 1: Compute firing rates on StemQAMixture biology."""
...
def main(
    ...
    output_base = base_dir / "outputs_scoping"
    ...
    if stage in ("all", "rank"):
        ranking, distribution = stage_rank(
            ...
            batch_size=batch_size * accum,  # can use larger batch for inference
            tokenizer=tokenizer,
            model=model,
            device=device,
            cache_dir=cache_dir,
    ...
    if stage in ("all", "recover"):
        ...
        stage_train(
            ...
            wandb_run="recover/biology_layer31_h0.0001",
