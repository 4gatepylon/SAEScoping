...
class LengthAwareCapableTokenizer:
    ...
    def _get_conversations_texts_tokens_type(
        ...
        assert not is_conversations or all(
            LengthAwareCapableTokenizer._sanity_check_single_turn_conversation(x) for x in conversations_or_texts_or_tokens
        )
    ...
    def length_aware_tokenize_conversations_or_texts_or_tokens(
        ...
        if _type == "tokens":
            ...
            assert sum(b["input_ids"].shape[0] for b in BEs) == len(tokens_with_indices), (
                f"belens={[b['input_ids'].shape for b in BEs]}, len(tokens)={len(tokens_with_indices)}"
            )
            assert all(len(i) == b["input_ids"].shape[0] for i, b in zip(indices, BEs)), (
    ...
    def regular_batched_tokenize_conversations_or_texts_or_tokens(
        ...
        if _type == "tokens":
            ...
            indices = [
                list(range(i, min(i + batch_size, len(conversations_or_texts_or_tokens))))
